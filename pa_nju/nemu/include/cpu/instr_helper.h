...
// Attention: inst_name can be any, but best to be the same as the real instruction name
// src_type could only be r, rm, i, a, c, o; dest_type could only be r, rm, a, o
// suffix could only be b, w, l, v, bv, short, near
...
// possible types: r(register), rm(e), i(imm), m(mem), 
// a(AL = AX = EAX), c(REG_CL == REG_CX == REG_ECX == 1), o(no modrm)
...
// e表示equal也表示0（因为equal通常是做减法得到）
#define condition_e \
	cpu.eflags.ZF
// a表示above，即CF=0且ZF=0
#define condition_a \
	(!cpu.eflags.CF && !cpu.eflags.ZF)
// ae表示above or equal，即CF=0
#define condition_ae \
	!cpu.eflags.CF
// b表示below，即CF=1
#define condition_b \
	cpu.eflags.CF
// be表示below or equal，即CF=1或ZF=1
#define condition_be \
	(cpu.eflags.CF || cpu.eflags.ZF)
// o表示overflow
#define condition_o \
...
#define condition_ne \
	!cpu.eflags.ZF
// na表示not above，即CF=1或ZF=1
#define condition_na \
	(cpu.eflags.CF || cpu.eflags.ZF)
// no表示not overflow，即OF=0
#define condition_no \
	!cpu.eflags.OF
// np表示not parity，即PF=0
#define condition_np \
	!cpu.eflags.PF
// ns表示not sign，即SF=0
#define condition_ns \
	!cpu.eflags.SF
// g表示greater，即ZF=0且SF=OF
#define condition_g \
	(!cpu.eflags.ZF && (cpu.eflags.SF == cpu.eflags.OF))
// ge表示greater or equal，即SF=OF
#define condition_ge \
	(cpu.eflags.SF == cpu.eflags.OF)
// l表示less，即SF!=OF
#define condition_l \
	(cpu.eflags.SF != cpu.eflags.OF)
// le表示less or equal，即ZF=1或SF!=OF
#define condition_le \
	(cpu.eflags.ZF || (cpu.eflags.SF != cpu.eflags.OF))
// ecxz表示ecx zero，即ecx=0
